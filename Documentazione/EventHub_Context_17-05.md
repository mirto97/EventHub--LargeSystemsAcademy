...
- MapStruct: **concesso**
- HTTP Basic + BCrypt: **requisito del file, rispettato**
...
### DTO
- `@Data @NoArgsConstructor @AllArgsConstructor` sui DTO è OK
...
### Security
- HTTP Basic con `Customizer.withDefaults()`
- Stateless (no sessione)
- CSRF disabilitato
- `DaoAuthenticationProvider(userDetailsService)` + `setPasswordEncoder()` — nota: in Spring Security 7 il costruttore vuole UserDetailsService, non no-args
...
> `@AssertTrue isEndDateAfterStartDate()` NON è sull'entity — la validazione cross-field è su `EventRequestDTO`.
...
public class EventService {
    ...
    public List<EventResponseDTO> getAllEvents() {
        return eventRepository.findAll().stream().map(this::toResponseWithSeats).toList();
    }
...
public class EventController {
    ...
    @GetMapping
    @Operation(summary = "Lista eventi pubblici")
...
Tutte le pagine si trovano in `static/html/`.
Tutte importano `/js/auth.js`.
Il frontend è accessibile da `http://localhost:8080/html/login.html`.

| File | Descrizione |
|---|---|
| `login.html` | Form login con HTTP Basic. Redirect per ruolo dopo login. |
| `signup.html` | Form registrazione. POST su /auth/signup. |
| `events.html` | Lista eventi pubblica con filtri per data e tag. |
| `event-detail.html` | Dettaglio evento, form prenotazione (se loggato e evento futuro), lista feedback e rating. |
| `my-bookings.html` | Lista biglietti utente. Cancella prenotazione. Form feedback per eventi conclusi. |
| `profile.html` | Visualizza e aggiorna il proprio profilo. POST se non esiste, PUT se esiste. |
| `organizer-events.html` | Lista eventi propri. Form crea/modifica evento con venue, tag, speaker. Lista partecipanti. |
| `admin.html` | Tab: Utenti (promuovi/banna/riattiva), Sedi (CRUD), Relatori (CRUD), Tag (crea/elimina). |
...
- [x] Controller REST completi con Swagger
- [x] Frontend HTML/CSS/JS vanilla (HTML e JS completati, CSS da fare)
- [ ] CSS frontend
- [ ] Test JUnit (EventService, TicketService)
